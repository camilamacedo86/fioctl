...
func doEdit(cmd *cobra.Command, args []string) {
	...
	if err != nil {
		logger.Log(logger.Error, "Unable to create tempfile: ", err)
		os.Exit(1)
	}
	defer os.Remove(tmpfile.Name())
	if _, err := tmpfile.Write(orig); err != nil {
		logger.Log(logger.Error, "Unable to write tempfile: ", err)
		os.Exit(1)
	}
	if err := tmpfile.Close(); err != nil {
		logger.Log(logger.Error, "Unable to close tempfile: ", err)
		os.Exit(1)
	}
	...
	if err != nil {
		logger.Log(logger.Error, "ERROR: Unable to re-read tempfile:", err)
	}
	...
	if err != nil {
		logger.Log(logger.Error, "Unable to parse new targets: ", err)
		os.Exit(1)
	}
	...
	if err != nil {
		logger.Log(logger.Error, "Unable to marshall targets data: ", err)
		os.Exit(1)
	}
